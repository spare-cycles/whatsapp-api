...
def resolve_lid(jid: str) -> ApiResponse:
    resolved = lid.normalize_jid(jid)
    return ApiResponse(
        success=True, data={"original": jid, "resolved": resolved}
    )


@router.get("/lid/map")
def get_lid_map() -> ApiResponse:
    return ApiResponse(success=True, data=lid.get_map())


@router.post("/lid/reload")
def reload_lid_map(settings: Settings = Depends(get_settings)) -> ApiResponse:
